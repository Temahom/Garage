...
<br> 
    {{-- @if (!isset($fournisseur->id)) --}}
        <form id="Formapprov" action="{{route('approvisionnements.update',['approvisionnement'=>$approvisionnement->id]) }}" method="POST">
            @csrf
    {{-- @else
        <form id="Formapprov" action="{{route('fournisseurs.approvisionnements.update',['fournisseur'=>$fournisseur->id]) }}" method="POST">
            @csrf
    @endif --}}
