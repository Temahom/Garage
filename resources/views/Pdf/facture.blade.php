<style>
    body{
        margin: 70px;
    }
    
</style>
<p style='line-height:normal;font-size:20px;font-family:montserrat;'>&nbsp;</p>
<table style="border-collapse:collapse;border:none;">
    <tbody>
        <tr>
            <td colspan="2" style="width:240.0px;border:none;height:232px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="font-size:32px;color:#7F7F7F;background:white;">FACTURE</span>
                </p>
            </td>
            <td style="border:none;height:232px;">
                <p style='line-height:normal;font-size:20px;font-family:montserrat;'>
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width: 100%;border: none;height: 132px;vertical-align: bottom;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:100%;border:none;height:232px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;">
                    <span><img width="102" src="http://garage-saka.herokuapp.com/assets/images/logo.png" alt="image"></span>
                </p>
            </td>
        </tr>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="color:#333F4F;margin-right: 300px;font-weight: bold;font-size: 25px;">SAKA</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:100%;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:100%;border:none;border-bottom:solid #BFBFBF 1px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;">
                    <strong>
                        <span style="font-family:montserrat;color:#1F3864;font-size: 20px;">{{$facture->created_at}}</span>
                    </strong>
                </p>
            </td>
        </tr>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="color:#333F4F;font-size: 22px;">Sector 32, Street 49, Choueifat - Lebanon</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:100%;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:100%;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
        </tr>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="font-family:montserrat;color:#333F4F;background:white;font-size: 22px;">77 719 93 89 <br>allouch.nabil@sakatrading.com</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:100%;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:100%;border:none;border-bottom:solid #BFBFBF 1px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;">
                    <strong>
                        <span style="font-family:montserrat;color:#1F3864;font-size: 25px;">N°:{{$facture->numero}}</span>
                    </strong>
                </p>
            </td>
        </tr>
    </tbody>
</table>
<p style="line-height:normal;font-size:20px;font-family:montserrat;">
    <span style="font-family:montserrat;">&nbsp;</span>
</p>
<table style="border-collapse:collapse;border:none;">
    <tbody>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td colspan="3" style="width:227.4px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
                <p style="line-height:115%;font-size:20px;font-family:montserrat;text-align:right;">
                    <em>
                        <span style="font-size:18px;line-height:  115%;font-family:montserrat;color:#333F4F;">&lt;</span>
                    </em>
                    <em>
                        <span style="font-size:18px;line-height:  115%;font-family:montserrat;color:#212121;background:white;">Conditions
                            de paiement, par exemple: d&ucirc; dans X jours</span>
                        </em>
                        <em>
                            <span style="line-height:  115%;font-family:montserrat;color:#333F4F;">&gt;</span>
                        </em>
                </p>
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:right;">
                    <em>
                        <span style="font-family:montserrat;color:#333F4F;">&nbsp;</span>
                    </em>
                </p>
            </td>
        </tr>
        <tr>
            <td style="width:210.65px;border:none;border-bottom:solid #BFBFBF 1px;height:15.0px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <strong>
                        <span style="color:#1F3864;background:white;margin-right: 300px;font-weight: bold;font-size: 25px;">DESTINATAIRE</span>
                    </strong>
                </p>
            </td>
            
        </tr>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="font-family:montserrat;font-size: 22px;">{{$client[0]->prenom}} {{$client[0]->nom}}</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            
        </tr>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="font-family:montserrat;font-size: 22px;">{{$client[0]->entreprise}} </span>
                </p>
            </td>
        </tr>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="font-size:22px;font-family:montserrat;">{{$client[0]->telephone}} </span>
                </p>
            </td>
            
        </tr>
        <tr>
            <td style="width:210.65px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="background:white;font-size: 22px;">{{$client[0]->email}} </span>
                </p>
            </td>
           
        </tr>
    </tbody>
</table>
<p style="line-height:normal;font-size:20px;font-family:montserrat;">
    <span style="font-family:montserrat;">&nbsp;</span>
</p>
<p style="line-height:normal;font-size:20px;font-family:montserrat;">
    <span style="font-family:montserrat;">&nbsp;</span>
</p>
<table style="border-collapse:collapse;border:none;margin: 20px;">
    <tbody>
        <tr style="height: 50px;text-align:center;">
            <td colspan="2" style="border:none;border-bottom:solid #BFBFBF 1px;background:#838181;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;width: 700px;">
                    <strong>
                        <span style="font-family:montserrat;color:white;">DESCRIPTION</span>
                    </strong>
                </p>
            </td>
            <td style="border:none;border-bottom:solid #BFBFBF 1px;background:#838181;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;width:250px;">
                    <strong>
                        <span style="color:white;">QUANTIT&Eacute;</span>
                    </strong>
                </p>
            </td>
            <td style="border:none;border-bottom:solid #BFBFBF 1px;background:#838181;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;width:250px;">
                    <strong>
                        <span style="color:white;">PRIX UNITAIRE (F CFA)</span>
                    </strong>
                </p>
            </td>
            <td style="border:none;border-bottom:solid #BFBFBF 1px;background:#838181;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <strong>
                        <span style="font-family:montserrat;color:white;">TOTAL (F CFA)</span>
                    </strong>
                </p>
            </td>
        </tr>
                         @php
                            $prixHT = 0;
                            $prixtotal = 0;
                            
                        @endphp

        @if (isset($les_devis))
            @foreach ($les_devis as $le_devi)
        
                    <tr style="height: 30px;text-align:center;">
                        <td colspan="2" style="border:solid #BFBFBF 1px;border-top:none;text-align: start;">
                            <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                                <span>{{ $le_devi->produit }}</span>
                            </p>
                        </td>
                        <td style="width:53.25px;border-top:none;border-left:none;border-bottom:solid #BFBFBF 1px;border-right:solid #BFBFBF 1px;">
                            <p style="line-height:normal;font-size:20px;font-family:montserrat;width:200px;">
                                <span>{{$le_devi->pivot->quantite }}</span>
                            </p>
                        </td>
                        <td style="border-top:none;border-left:none;border-bottom:solid #BFBFBF 1px;border-right:solid #BFBFBF 1px;">
                            <p style="line-height:normal;font-size:20px;font-family:montserrat;width:200px;">
                                <span>{{ number_format($le_devi->prix1, 0, ',', ' ') }}</span>
                            </p>
                        </td>
                        @php
                            $prixHT += $le_devi->pivot->quantite * $le_devi->prix1;
                        
                         @endphp
                        <td style="border-top:none;border-left:none;border-bottom:solid #BFBFBF 1px;border-right:solid #BFBFBF 1px;">
                            <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;width:200px;">
                                <span>{{ number_format($le_devi->pivot->quantite * $le_devi->prix1, 0, ',', ' ') }}</span>
                            </p>
                        </td>
                    </tr>
            @endforeach
        @endif
        <tr style="height: 30px;text-align:center;">
            <td style="width:234.0px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width:6.0px;border:none;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width: 53.25px;border: none;height: 19px;vertical-align: bottom;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:15px;font-family:montserrat;text-align:right;">
                    <strong>
                        <span style="font-family:montserrat;color:#333F4F;background:white;margin-right: 12px;">SOUS-TOTAL</span>
                    </strong>
                </p>
            </td>
            <td style="width:100%;border:none;border-bottom:solid #BFBFBF 1px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;">
                    <span style="font-family:montserrat;">{{ number_format($prixHT, 0, ',', ' ') }}</span>
                </p>
            </td>
        </tr>
        <tr style="height: 30px;text-align:center;">
            <td colspan="2" rowspan="4" style="border:none;text-align: start;font-weight:bold;text-decoration: underline;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span style="font-size:18px;background:white;">Remarques, notes et instructions de paiement:</span>
                </p>
            </td>
            <td style="width: 53.25px;border: none;height: 19px;vertical-align: bottom;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            @if (isset($devi))

                <td style="border:none;">
                    <p style="line-height:normal;font-size:15px;font-family:montserrat;text-align:right;">
                        <strong>
                            <span style="font-family:montserrat;color:#333F4F;margin-right: 12px;">COUT DE REPARATION</span>
                        </strong>
                    </p>
                </td>
                <td style="width:100%;border:none;border-bottom:solid #BFBFBF 1px;">
                    <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;">
                        <span style="font-family:montserrat;">{{ number_format($devi->cout, 0, ',', ' ') }}</span>
                    </p>
                </td>
           
            @endif
        </tr>
        <tr style="height: 30px;text-align:center;">
            <td style="width: 53.25px;border: none;height: 19px;vertical-align: bottom;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:15px;font-family:montserrat;text-align:right;">
                    <strong>
                        <span style="font-family:montserrat;color:#333F4F;">COUT DIAGNOSTIC</span>
                    </strong>
                </p>
            </td>
            <td style="width:100%;border:none;border-bottom:solid #BFBFBF 1px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;">
                    <span style="font-family:montserrat;">{{ number_format($prix_total, 0, ',', ' ') }}</span>
                </p>
            </td>
        </tr>
        <tr style="height: 30px;text-align:center;">
            <td style="width: 53.25px;border: none;height: 19px;vertical-align: bottom;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;">
                <p style="line-height:normal;font-size:15px;font-family:montserrat;text-align:right;">
                    <strong>
                        <span style="font-family:montserrat;color:#333F4F;margin-right: 12px;">TAXE TOTAL</span>
                    </strong>
                </p>
            </td>
            <td style="width:100%;border:none;border-bottom:solid #BFBFBF 1px;">
                @php
                $cout=0;
                    if(isset($devi)){
                        $cout= $devi->cout;
                    }
                @endphp
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:center;">
                    <span style="font-family:montserrat;">{{ number_format($prix_total+$prixHT+$cout, 0, ',', ' ') }}</span>
                </p>
            </td>
        </tr>
        <tr style="height: 30px;text-align:center;">
            <td style="width: 53.25px;border: none;height: 19px;vertical-align: bottom;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
        </tr>
        <tr style="height: 30px;text-align:center;">
            <td colspan="2" style="width:240.0px;border:none;height:33.0px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="width: 53.25px;border: none;height: 33px;vertical-align: bottom;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;">
                    <span>&nbsp;</span>
                </p>
            </td>
            <td style="border:none;height:33.0px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:right;">
                    <strong>
                        <span style="font-size:16px;font-family:montserrat;color:#333F4F;margin-right: 12px;">SOLDE DU</span>
                    </strong>
                </p>
            </td>
            <td style="width:100%;border:none;border-bottom:solid black 1px;background:#eaead3;height:33.0px;">
                <p style="line-height:normal;font-size:20px;font-family:montserrat;text-align:right;">
                    <strong>
                        <span style="font-size:16px;font-family:montserrat;">{{ number_format($prix_total+$prixHT+$cout, 0, ',', ' ') }}</span>
                    </strong>
                </p>
            </td>
        </tr>
        <tr style="height: 30px;text-align:center;">
            <td style="border:none;"><br></td>
            <td style="border:none;"><br></td>
            <td style="border:none;"><br></td>
            <td style="border:none;"><br></td>
            <td style="border:none;"><br></td>
        </tr>
    </tbody>
</table>
<p style="line-height:115%;font-size:20px;font-family:montserrat;">
    <span style="line-height:115%;font-family:montserrat;">&nbsp;</span>
</p>
